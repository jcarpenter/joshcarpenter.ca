(function (window, document) {
    ...
    let article, popup
    let intervalTick = 100 // ms
    let enterTimer, exitTimer // SetIntervals
    let isEnterTimerActive, isExitTimerActive // bool
    let enterDelay = 100 // ms
    let exitDelay = 150 // ms
    let enterElapsed = 0
    let exitElapsed = 0
    let isPopupOpen
    let isMouseDown = 0
    ...
    function makePopup() {
        ...
        popup.classList.add('popup-hide', 'footnote-item') // Hide popup to start
    ...
    function updatePopup(link) {
        ...
        let citation = article.querySelector(href + '> p')
        let clone = citation.cloneNode(true)
        let backLink = clone.querySelector('.footnote-backref')
    ...
    function setupLinks() {
        ...
        let links = article.querySelectorAll('.footnote-ref')
